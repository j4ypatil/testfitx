...
app.post('/api/tdee', (req, res) => {
  ...
  const adj = { lose_fat: -500, build_muscle: 300, maintain: 0, body_recomp: 0 };
  res.json({ bmr, tdee, adjustedCalories: tdee + (adj[goalType] || 0) });
});

app.post('/api/scan-food', async (req, res) => {
  const { image } = req.body;
  if (!image) return res.json({ ok: false, error: 'No image' });
  try {
...
- If the image has no recognizable food, return [{"food_name":"Unknown","quantity":1,"unit":"serving","calories":0}]`;
    ...
    const geminiRes = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${apiKey}`, {
      ...
      body: JSON.stringify({ contents: [{ parts: [{ inline_data: { mime_type: 'image/jpeg', data: image } }, { text: prompt }] }] }),
    });
    ...
    if (!Array.isArray(items) || items.length === 0) return res.json({ ok: false, error: 'Could not identify food' });
...
async function fetchExerciseById(apiKey, id) {
  ...
  const apiRes = await fetch(url, { headers: { 'x-rapidapi-key': apiKey, 'x-rapidapi-host': 'exercisedb.p.rapidapi.com' } });
  return apiRes.json();
}
...
app.get('/api/exercise-details/:name', async (req, res) => {
  ...
  if (!mappedId) return res.json({ ok: false, error: 'Not found' });
  try {
    const data = await fetchExerciseById(apiKey, mappedId);
    res.json({ ok: true, id: mappedId, gifUrl: data.gifUrl, target: data.target, equipment: data.equipment, instructions: data.instructions || [] });
  } catch { res.json({ ok: false, error: 'Fetch failed' }); }
});

app.get('/api/exercise-mapping', (req, res) => res.json(exerciseMapping));

// Proxy ExerciseDB exercise images
...
OUTPUT JSON SCHEMA:
{
  ...
  try {
    ...
    if (!days || days.length === 0) return res.json({ ok: false, error: 'Invalid plan format' });
    res.json({ ok: true, plan: days });
  } catch (err) { res.json({ ok: false, error: err.message }); }
});

app.post('/api/analyze-photo', async (req, res) => {
  const { image, onboarding } = req.body;
  if (!image) return res.json({ ok: false, error: 'No image' });
  const apiKey = (process.env.GEMINI_API_KEY || FALLBACK_GEMINI);
  if (!apiKey) return res.json({ ok: false, error: 'No API key' });

  const prompt = `SYSTEM: You are an expert physique analyst and personal trainer. Given a progress photo of someone (${onboarding?.bodyType || 'fitness'} body type, ${onboarding?.gender || 'unknown'}), analyze their current physique and identify 2-4 priority muscle groups that need the most improvement based on their target body type. Return ONLY this JSON (no markdown, no backticks): 
{"focusAreas":["MuscleGroup1","MuscleGroup2"],"reasoning":"brief reason","intensity":"mild|moderate|high"}`;
