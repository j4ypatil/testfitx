...
export default function LongTermResultsScreen({ onBack, onContinue, stat = '80%', statText = 'of fitcal users maintain their weight loss even 6 months later' }) {
  return (
    <div className="min-h-screen bg-black flex flex-col max-w-sm mx-auto p-6">
      {/* 1. Header */}
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="w-20 h-1 bg-white/10 rounded-full overflow-hidden">
          <div className="w-1/6 h-full bg-white rounded-full" />
        </div>
      </div>

      {/* 2. Headline */}
      <h1 className="text-[40px] leading-[1.1] font-bold text-white mb-8">
        fitcal creates<br />long-term results
      </h1>

      {/* 3. Card */}
      <div className="bg-[#1c1c1e] rounded-3xl p-6 mb-8">
        <h2 className="text-2xl font-bold text-white mb-6">Your weight</h2>
        ...
        <div className="h-48 w-full mb-6">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data} margin={{ top: 5, right: 5, left: -20, bottom: 5 }}>
              ...
              <Line type="monotone" dataKey="fitcal" stroke="#ffffff" strokeWidth={3} dot={{r: 4, fill: '#ffffff'}} />
              <Line type="monotone" dataKey="diet" stroke="#f87171" strokeWidth={3} dot={{r: 4, fill: '#f87171'}} />
            </LineChart>
          </ResponsiveContainer>
        </div>

        <p className="text-center text-white/50 text-sm leading-relaxed">
      ...
      {/* 4. Bottom Button */}
      <button onClick={onContinue} className="mt-auto w-full py-4 rounded-full bg-white text-black font-bold text-lg hover:bg-white/90 transition-all">
        Continue
      </button>
    </div>
  );
